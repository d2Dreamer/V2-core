...
module yuzu_framework::deployer {
    ...
    /// The account passed is not the yuzu account
    const ERROR_INVALID_YUZU_ACCOUNT: u64 = 0;
    ...
    entry public fun init(yuzu_framework: &signer, fee: u64, owner: address){
        assert!(signer::address_of(yuzu_framework) == @yuzu_framework, ERROR_INVALID_YUZU_ACCOUNT);
        move_to(yuzu_framework, Config { owner, fee })
    }

    entry public fun update_fee(yuzu_framework: &signer, new_fee: u64) acquires Config {
        assert!(
            signer::address_of(yuzu_framework) == @yuzu_framework, 
            ERROR_INVALID_YUZU_ACCOUNT
        );
        // only allowed after the deployer is initialized
        let config = borrow_global_mut<Config>(@yuzu_framework);
        config.fee = new_fee;
    ...
    entry public fun update_fee_account(signer_ref: &signer, new_fee_account: address) acquires Config {
        assert!(
            signer::address_of(signer_ref) == @yuzu_framework, 
            ERROR_INVALID_YUZU_ACCOUNT
        );
        let config = borrow_global_mut<Config>(@yuzu_framework);
    ...
    entry public fun generate_coin_with_caps<CoinType>(
        ...
        // only allowed after the deployer is initialized
        assert!(exists<Config>(@yuzu_framework), ERROR_INVALID_YUZU_ACCOUNT);
        // the deployer must have enough APT to pay for the fee
        assert!(
            coin::balance<AptosCoin>(signer::address_of(deployer)) >= borrow_global<Config>(@yuzu_framework).fee,
    ...
    public fun get_fee(): u64  acquires Config {
        borrow_global<Config>(@yuzu_framework).fee
    }

    #[view]
    // get fee account
    public fun get_fee_account(): address  acquires Config {
        borrow_global<Config>(@yuzu_framework).owner
    }
    ...
    #[test_only]
    public fun init_test(yuzu_framework: &signer, fee: u64, owner: address) {
        assert!(
            signer::address_of(yuzu_framework) == @yuzu_framework, 
            ERROR_INVALID_YUZU_ACCOUNT
        );
    ...
    #[test(aptos_framework = @0x1, yuzu_framework = @yuzu_framework)]
    // #[expected_failure, code = 65537]
    fun test_user_deploys_coin(
        aptos_framework: signer,
        yuzu_framework: signer
    ) acquires Config {
        aptos_framework::account::create_account_for_test(signer::address_of(&yuzu_framework));
        // aptos_framework::account::create_account_for_test(signer::address_of(user));
        init(&yuzu_framework, 1, signer::address_of(&yuzu_framework));
        assert!(get_fee() == 1, 0);
        ...
        coin::register<AptosCoin>(&yuzu_framework);
        // mint some APT to be able to pay for the fee of generate_coin
        aptos_coin::mint(&aptos_framework, signer::address_of(&yuzu_framework), 1000);
        
        generate_coin<Fakeyuzu>(
            &yuzu_framework,
            string::utf8(b"Fake yuzu"),
            string::utf8(b"yuzu"),
            4,
            1000000,
            true,
        ...
        // assert Fakeyuzu is generated and supply is moved under the deployer's wallet
        assert!(coin::balance<Fakeyuzu>(signer::address_of(&yuzu_framework)) == 1000000, 1);

        // assert coins table contains the newly created coin
        assert!(is_coin_owner<Fakeyuzu>(&yuzu_framework), 1);
    }

    #[test(aptos_framework = @0x1, yuzu_framework = @yuzu_framework, user = @0x123, new_yuzu = @0x456)]
    public fun test_fee(
        aptos_framework: signer,
        yuzu_framework: signer,
        user: signer,
        ...
        let new_yuzu_addr = signer::address_of(&new_yuzu);
        aptos_framework::account::create_account_for_test(signer::address_of(&yuzu_framework));
        aptos_framework::account::create_account_for_test(signer::address_of(&user));
        aptos_framework::account::create_account_for_test(new_yuzu_addr);
        // features::change_feature_flags(&aptos_framework, vector[26], vector[]);
        // aptos_framework::account::create_account_for_test(signer::address_of(user));
        init(&yuzu_framework, 100, signer::address_of(&yuzu_framework));
        assert!(get_fee() == 100, 0);
        update_fee(&yuzu_framework, 200);
        assert!(get_fee() == 200, 0);
        update_fee_account(&yuzu_framework, new_yuzu_addr);
        assert!(
            borrow_global<Config>(@yuzu_framework).owner == new_yuzu_addr,
            0
        );
        ...
        // register aptos coin and mint some APT to be able to pay for the fee of generate_coin
        let (aptos_coin_burn_cap, aptos_coin_mint_cap) = aptos_coin::initialize_for_test(&aptos_framework);
        coin::register<AptosCoin>(&new_yuzu);
        coin::register<AptosCoin>(&yuzu_framework);
        // mint some APT to be able to pay for the fee of generate_coin
        aptos_coin::mint(&aptos_framework, signer::address_of(&yuzu_framework), 10000000);
        ...
        debug::print<u64>(&coin::balance<AptosCoin>(new_yuzu_addr));
        assert!(coin::balance<AptosCoin>(new_yuzu_addr) == 200, 1);
