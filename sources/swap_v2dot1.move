...
module yuzu_v2dot1::swap_v2dot1 {
    ...
    inline fun temp_toggle_fee_on_transfer_fees<CoinType, X, Y>(activate: bool) acquires TokenPairMetadata {
        ...
        assert!(is_fee_on_transfer_registered<CoinType, X, Y>(), errors_v2dot1::fee_on_transfer_not_registered());

        let metadata = borrow_global_mut<TokenPairMetadata<X, Y>>(constants_v2dot1::get_resource_account_address());

        if (activate) {
            metadata.liquidity_fee = metadata.liquidity_fee + fee_on_transfer_v2dot1::get_liquidity_fee<CoinType>();
            metadata.team_fee = metadata.team_fee + fee_on_transfer_v2dot1::get_team_fee<CoinType>();
            metadata.rewards_fee = metadata.rewards_fee + fee_on_transfer_v2dot1::get_rewards_fee<CoinType>();
        } else {
            metadata.liquidity_fee = metadata.liquidity_fee - fee_on_transfer_v2dot1::get_liquidity_fee<CoinType>();
            metadata.team_fee = metadata.team_fee - fee_on_transfer_v2dot1::get_team_fee<CoinType>();
            metadata.rewards_fee = metadata.rewards_fee - fee_on_transfer_v2dot1::get_rewards_fee<CoinType>();
    ...
    public entry fun toggle_rewards_fee<CoinType, X, Y>(
        ...
        assert!(
            ...
            errors_v2dot1::coin_type_does_not_match_x_or_y()
        );
        // assert sender is token owner
        assert!(deployer::is_coin_owner<CoinType>(sender), errors_v2dot1::not_owner());
        // assert fee on transfer is registered
    ...
    public(friend) fun create_pair<X, Y>(
        ...
        assert!(!is_pair_created<X, Y>(), errors_v2dot1::already_initialized());
    ...
    public(friend) fun add_fee_on_transfer_in_pair<CoinType, X, Y>(
        ...
        if (type_info::type_of<CoinType>() == type_info::type_of<X>()) {
            let fee_on_transfer = fee_on_transfer_v2dot1::get_info<X>();
        ...
        // if Cointype = Y, add fee_on_transfer to pair_metadata.fee_on_transfer_y
            ...
            let fee_on_transfer = fee_on_transfer_v2dot1::get_info<Y>();
    ...
    fun swap_exact_x_to_y_direct<X, Y>(
        ...
        let amount_out = swap_utils_v2dot1::get_amount_out(amount_in, rin, rout, liquidity_fee<X, Y>());
        let (coins_x_out, coins_y_out) = swap<X, Y>(0, amount_out);
        assert!(coin::value<X>(&coins_x_out) == 0, errors_v2dot1::insufficient_output_amount());
    ...
    public(friend) fun swap_y_to_exact_x_direct<X, Y>(
        ...
        assert!(coin::value<Y>(&coins_y_out) == 0, errors_v2dot1::insufficient_output_amount());
        (coins_x_out, coins_y_out)
    }
    ...
    fun swap_exact_y_to_x_direct<X, Y>(
        ...
        let metadata = borrow_global<TokenPairMetadata<X, Y>>(constants_v2dot1::get_resource_account_address());
        let liquidity_fee = metadata.liquidity_fee;
        let amount_out = swap_utils_v2dot1::get_amount_out(amount_in, rin, rout, liquidity_fee);  
        let (coins_x_out, coins_y_out) = swap<X, Y>(amount_out, 0);
        assert!(coin::value<Y>(&coins_y_out) == 0, errors_v2dot1::insufficient_output_amount());
        (coins_x_out, coins_y_out)
    }
    ...
    public fun is_pair_created<X, Y>(): bool {
        exists<TokenPairReserve<X, Y>>(constants_v2dot1::get_resource_account_address())
    }
    ...
    public fun token_reserves<X, Y>(): (u64, u64, u64) acquires TokenPairReserve {
        let reserve = borrow_global<TokenPairReserve<X, Y>>(constants_v2dot1::get_resource_account_address());
    ...
    public fun token_balances<X, Y>(): (u64, u64) acquires TokenPairMetadata {
        let meta =
            borrow_global<TokenPairMetadata<X, Y>>(constants_v2dot1::get_resource_account_address());
        (
    ...
    public fun get_reserve<X, Y>(): TokenPairReserve<X, Y> acquires TokenPairReserve {
        ...
        assert!(is_pair_created<X, Y>(), errors_v2dot1::pair_not_created());
        let reserve = borrow_global<TokenPairReserve<X, Y>>(constants_v2dot1::get_resource_account_address());
    ...
    public fun is_fee_on_transfer_registered<CoinType, X, Y>(): bool acquires TokenPairMetadata {
        ...
        if (swap_utils_v2dot1::sort_token_type<X, Y>()) {
            ...
            // assert pair is created
            assert!(is_pair_created<Y, X>(), errors_v2dot1::pair_not_created());
            let metadata = borrow_global<TokenPairMetadata<Y, X>>(constants_v2dot1::get_resource_account_address());
            // if CoinType = Y
    ...
    public fun get_dex_fees_in_a_pair<X, Y>(): (u128, u128) acquires TokenPairMetadata {
        if (swap_utils_v2dot1::sort_token_type<X, Y>()) {
            ...
            if (is_fee_on_transfer_registered<X, X, Y>()) {
                let fee_on_transfer_liquidity_fee = fee_on_transfer_v2dot1::get_liquidity_fee<X>();
            ...
            if (is_fee_on_transfer_registered<Y, X, Y>()) {
                let fee_on_transfer_liquidity_fee = fee_on_transfer_v2dot1::get_liquidity_fee<Y>();
                dex_liquidity_fee = dex_liquidity_fee - fee_on_transfer_liquidity_fee;
            };
    ...
    inline fun calculate_fee_on_transfer_amounts<CoinType>(amount: u64): (u128, u128, u128) {
        ...
        (
            utils_v2dot1::calculate_amount(fee_on_transfer_v2dot1::get_liquidity_fee<CoinType>(), amount),
            utils_v2dot1::calculate_amount(fee_on_transfer_v2dot1::get_rewards_fee<CoinType>(), amount),
            utils_v2dot1::calculate_amount(fee_on_transfer_v2dot1::get_team_fee<CoinType>(), amount)
        )
    }
    ...
    inline fun calculate_dex_fees_amounts<CoinType>(amount_in: u64): (u128, u128) {
        // calculate fee amounts
        (
            utils_v2dot1::calculate_amount(admin_v2dot1::get_liquidity_fee_modifier(), amount_in),
            utils_v2dot1::calculate_amount(admin_v2dot1::get_treasury_fee_modifier(), amount_in)
        )
    }
    ...
    fun add_liquidity_direct<X, Y>(
        ...
        let (a_x, a_y) = if (reserve_x == 0 && reserve_y == 0) {
            ...
            let amount_y_optimal = swap_utils_v2dot1::quote(amount_x, reserve_x, reserve_y);
            if (amount_y_optimal <= amount_y) {
                (amount_x, amount_y_optimal)
            } else {
                let amount_x_optimal = swap_utils_v2dot1::quote(amount_y, reserve_y, reserve_x);
                assert!(amount_x_optimal <= amount_x, errors_v2dot1::invalid_amount());
                (amount_x_optimal, amount_y)
            }
        };

        assert!(a_x <= amount_x, errors_v2dot1::insufficient_amount());
        assert!(a_y <= amount_y, errors_v2dot1::insufficient_amount());
    ...
    fun distribute_fee_on_transfer_fees<CoinType, X, Y>(signer_ref: &signer, amount: u64) acquires TokenPairMetadata {
        ...
        */ 
        if (type_info::type_of<CoinType>() == type_info::type_of<X>()) {
            ...
            if (stake_v2dot1::is_pool_created<X, Y>()) {
                ...
                stake_v2dot1::distribute_rewards<X, Y>(rewards_coins_to_xy_pool, coin::zero<Y>());
            };
            if (stake_v2dot1::is_pool_created<Y, X>()) {
                let rewards_coins_to_yx_pool = coin::withdraw<X>(signer_ref, (x_rewards_amount_from_y_ratio as u64));
                stake_v2dot1::distribute_rewards<Y, X>(coin::zero<Y>(), rewards_coins_to_yx_pool);
            };
            // team
            if (is_fee_on_transfer_registered<X, X, Y>()) {
                let x_team_x_coins = coin::withdraw<X>(signer_ref, (x_team_amount_from_x_ratio as u64));
                aptos_account::deposit_coins<X>(fee_on_transfer_v2dot1::get_owner<X>(), x_team_x_coins);
            };
            if (is_fee_on_transfer_registered<Y, X, Y>()) {
                let y_team_x_coins = coin::withdraw<X>(signer_ref, (x_team_amount_from_y_ratio as u64));
                aptos_account::deposit_coins<X>(fee_on_transfer_v2dot1::get_owner<Y>(), y_team_x_coins);
            };
        }
        /*
        ...
        */ 
            ...
            if (stake_v2dot1::is_pool_created<X, Y>()) {
                ...
                stake_v2dot1::distribute_rewards<X, Y>(coin::zero<X>(), rewards_coins_to_xy_pool);
            };
            if (stake_v2dot1::is_pool_created<Y, X>()) {
                let rewards_coins_to_yx_pool = coin::withdraw<Y>(signer_ref, (y_rewards_amount_from_y_ratio as u64));
                stake_v2dot1::distribute_rewards<Y, X>(rewards_coins_to_yx_pool, coin::zero<X>());
            };
            // team
            if (is_fee_on_transfer_registered<X, X, Y>()) {
                let x_team_y_coins = coin::withdraw<Y>(signer_ref, (y_team_amount_from_x_ratio as u64));
                aptos_account::deposit_coins<Y>(fee_on_transfer_v2dot1::get_owner<X>(), x_team_y_coins);
            };
            if (is_fee_on_transfer_registered<Y, X, Y>()) {
                let y_team_y_coins = coin::withdraw<Y>(signer_ref, (y_team_amount_from_y_ratio as u64));
                aptos_account::deposit_coins<Y>(fee_on_transfer_v2dot1::get_owner<Y>(), y_team_y_coins);
            };
        }
    }
    ...
    public(friend) fun update_fee_tier<Tier, X, Y>(signer_ref: &signer) acquires TokenPairMetadata {
        ...
        assert!(signer::address_of(signer_ref) == admin_v2dot1::get_admin(), errors_v2dot1::not_admin());
        // assert tier is valid
        assert!(admin_v2dot1::is_valid_tier<Tier>(), errors_v2dot1::invalid_tier());
        // update fees
        ...
        let metadata = borrow_global_mut<TokenPairMetadata<X, Y>>(constants_v2dot1::get_resource_account_address());
        if (type_info::type_of<Tier>() == type_info::type_of<admin_v2dot1::Universal>()) {
            // get the fees
            let (liquidity_fee, treasury_fee) = admin_v2dot1::get_universal_tier_fees();
            // update pair fees 
            metadata.liquidity_fee = liquidity_fee;
            metadata.treasury_fee = treasury_fee;
        } else if (type_info::type_of<Tier>() == type_info::type_of<admin_v2dot1::PopularTraded>()) {
            // get the fees
            let (liquidity_fee, treasury_fee) = admin_v2dot1::get_popular_traded_tier_fees();
            // update pair fees 
            metadata.liquidity_fee = liquidity_fee;
            metadata.treasury_fee = treasury_fee;
        } else if (type_info::type_of<Tier>() == type_info::type_of<admin_v2dot1::Stable>()) {
            // get the fees
            let (liquidity_fee, treasury_fee) = admin_v2dot1::get_stable_tier_fees();
            // update pair fees 
            metadata.liquidity_fee = liquidity_fee;
            metadata.treasury_fee = treasury_fee;
        } else {
            // get the fees
            let (liquidity_fee, treasury_fee) = admin_v2dot1::get_very_stable_tier_fees();
            // update pair fees 
            metadata.liquidity_fee = liquidity_fee;
            metadata.treasury_fee = treasury_fee;
        };
    ...
    fun swap<X, Y>(
        ...
        assert!(amount_x_out > 0 || amount_y_out > 0, errors_v2dot1::insufficient_output_amount());

        let reserves = borrow_global_mut<TokenPairReserve<X, Y>>(constants_v2dot1::get_resource_account_address());
        assert!(amount_x_out < reserves.reserve_x && amount_y_out < reserves.reserve_y, errors_v2dot1::insufficient_liquidity());
        ...
        assert!(amount_x_in > 0 || amount_y_in > 0, errors_v2dot1::insufficient_input_amount());

        let prec = (constants_v2dot1::get_precision() as u128);
        ...
        // No need to use u256 when balance_x_adjusted * balance_y_adjusted and reserve_x_adjusted * reserve_y_adjusted are less than constants_v2dot1::get_max_u128().
        let compare_result = if(balance_x_adjusted > 0 && reserve_x_adjusted > 0 && constants_v2dot1::get_max_u128() / balance_x_adjusted > balance_y_adjusted && constants_v2dot1::get_max_u128() / reserve_x_adjusted > reserve_y_adjusted){
        ...
        assert!(compare_result, errors_v2dot1::k());
    ...
    fun mint<X, Y>(): (Coin<LPToken<X, Y>>) acquires TokenPairReserve, TokenPairMetadata {
        ...
        let liquidity = if (total_supply == 0u128) {
            ...
            assert!(sqrt > constants_v2dot1::get_minimum_liquidity(), errors_v2dot1::insufficient_liquidity_minted());
            let l = sqrt - constants_v2dot1::get_minimum_liquidity();
            // permanently lock the first minimum liquidity tokens
            mint_lp_to<X, Y>(constants_v2dot1::get_resource_account_address(), (constants_v2dot1::get_minimum_liquidity() as u64), &metadata.mint_cap);
            l
        } else {
            let liquidity = math::min(amount_x * total_supply / (reserves.reserve_x as u128), amount_y * total_supply / (reserves.reserve_y as u128));
            assert!(liquidity > 0u128, errors_v2dot1::insufficient_liquidity_minted());
            liquidity
        };
    ...
    fun burn<X, Y>(lp_tokens: Coin<LPToken<X, Y>>): (Coin<X>, Coin<Y>) acquires TokenPairMetadata, TokenPairReserve {
        ...
        assert!(amount_x > 0 && amount_y > 0, errors_v2dot1::insufficient_liquidity_burned());
    ...
    fun deposit_x<X, Y>(amount: Coin<X>) acquires TokenPairMetadata {
        let metadata =
            borrow_global_mut<TokenPairMetadata<X, Y>>(constants_v2dot1::get_resource_account_address());
    ...
    fun extract_x<X, Y>(amount: u64, metadata: &mut TokenPairMetadata<X, Y>): Coin<X> {
        assert!(coin::value<X>(&metadata.balance_x) > amount, errors_v2dot1::insufficient_amount());
        coin::extract(&mut metadata.balance_x, amount)
    }

    // Extract `amount` from this contract
    fun extract_y<X, Y>(amount: u64, metadata: &mut TokenPairMetadata<X, Y>): Coin<Y> {
        assert!(coin::value<Y>(&metadata.balance_y) > amount, errors_v2dot1::insufficient_amount());
