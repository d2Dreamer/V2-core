...
module yuzu_v2dot1::swap_v2dot1_test {
    ...
    fun test_ownership_transfer(
        ...
        admin_v2dot1::claim_admin_previliges(bob);
        assert!(admin_v2dot1::get_admin() == signer::address_of(bob), 1);

        // transfer admin_v2dot1:: previliges back to admin_v2dot1::
        admin_v2dot1::offer_admin_previliges(bob, signer::address_of(admin));
        admin_v2dot1::claim_admin_previliges(admin);
        assert!(admin_v2dot1::get_admin() == signer::address_of(admin), 2);

        // transfer admin_v2dot1:: previliges to alice but alice rejects it
        admin_v2dot1::offer_admin_previliges(admin, signer::address_of(alice));
        admin_v2dot1::reject_admin_previliges(alice);
        assert!(admin_v2dot1::get_admin() == signer::address_of(admin), 3);

        // transfer treasury previliges to alice
        admin_v2dot1::offer_treasury_previliges(admin, signer::address_of(alice));
        admin_v2dot1::claim_treasury_previliges(alice);
        assert!(admin_v2dot1::get_treasury_address() == signer::address_of(alice), 4);

        // transfer treasury previliges to bob but bob rejects it
        admin_v2dot1::offer_treasury_previliges(admin, signer::address_of(bob));
        admin_v2dot1::reject_treasury_previliges(bob);
    ...
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(bob, bob_liquidity_x, bob_liquidity_y, 0, 0);
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(alice, alice_liquidity_x, alice_liquidity_y, 0, 0);

    //     let (balance_y, balance_x) = swap_v2dot1::token_balances<TestBUSD, TestCAKE>();
    //     let (reserve_y, reserve_x, _) = swap_v2dot1::token_reserves<TestBUSD, TestCAKE>();
    ...
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 0, 0);
    ...
    //     // bob provider liquidity for 5:10 CAKE-BUSD
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(bob, initial_reserve_x, initial_reserve_y, 0, 0);
    ...
    //     // bob provider liquidity for 5:10 CAKE-BUSD
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(bob, bob_add_liquidity_x, bob_add_liquidity_y, 0, 0);
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(alice, alice_add_liquidity_x, alice_add_liquidity_y, 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(bob, (bob_suppose_lp_balance as u64), 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(alice, (alice_suppose_lp_balance as u64), 0, 0);
    ...
    //     // bob provider liquidity for 5:10 CAKE-BUSD
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(user1, user1_add_liquidity_x, user1_add_liquidity_y, 0, 0);
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(user2, user2_add_liquidity_x, user2_add_liquidity_y, 0, 0);
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(user3, user3_add_liquidity_x, user3_add_liquidity_y, 0, 0);
    //     router_v2dot1::add_liquidity<TestCAKE, TestBUSD>(user4, user4_add_liquidity_x, user4_add_liquidity_y, 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(user1, (user1_suppose_lp_balance as u64), 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(user2, (user2_suppose_lp_balance as u64), 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(user3, (user3_suppose_lp_balance as u64), 0, 0);
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(user4, (user4_suppose_lp_balance as u64), 0, 0);
    ...
    //     let (balance_y, balance_x) = swap_v2dot1::token_balances<TestBUSD, TestCAKE>();
    //     let (reserve_y, reserve_x, _) = swap_v2dot1::token_reserves<TestBUSD, TestCAKE>();
    //     let total_supply = swap_v2dot1::total_lp_supply<TestBUSD, TestCAKE>();
    ...
    //     router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(bob, bob_lp_balance, 0, 0);
    ...
    //     let (treasury_balance_x, treasury_balance_y, team_balance_x, team_balance_y, pool_balance_x, pool_balance_y) = swap_v2dot1::token_fees_accumulated<TestBUSD, TestCAKE>();
    ...
    //     let total_fees = swap_v2dot1::token_fees<TestBUSD, TestCAKE>();
    ...
    //     // swap_v2dot1::withdraw_fee<TestCAKE, TestBUSD>(treasury);
    //     // let treasury_lp_after_balance = coin::balance<LPToken<TestBUSD, TestCAKE>>(signer::address_of(treasury));
    //     // router_v2dot1::remove_liquidity<TestCAKE, TestBUSD>(treasury, (suppose_fee_amount as u64), 0, 0);
    ...
    //     let x_in_amount = router_v2dot1::get_amount_in<TestCAKE, TestBUSD>(output_y);
    //     assert!(x_in_amount == (input_x as u64), 102);

    //     let input_x_coin = coin::withdraw(alice, (input_x as u64));

    //     let (x_out, y_out) =  router_v2dot1::swap_x_to_exact_y_direct_external<TestCAKE, TestBUSD>(input_x_coin, output_y);
    ...
    //     let y_in_amount = router_v2dot1::get_amount_in<TestBUSD, TestCAKE>(output_x);
    ...
    //     let reserve_y;
    //     if(is_x_to_y){
    //         (reserve_x, reserve_y, _) = swap_v2dot1::token_reserves<X, Y>();
    //     }else{
    //         (reserve_y, reserve_x, _) = swap_v2dot1::token_reserves<Y, X>();
    //     };
    //     (reserve_x, reserve_y)
